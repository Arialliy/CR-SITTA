#!/usr/bin/env python3
"""Run the train-only P3 Stage-B1 v2 gradient decomposition.
...
from analysis.foreground_background_gradient_decomposition_v2 import (  # noqa: E402
    BACKWARD_BASIS,
...
from analysis.p3_stage_b1_contract_v2 import (  # noqa: E402
    CONFIG_FILE_SHA256,
...
from analysis.p3_stage_b1_aggregate_v2 import (  # noqa: E402
...
from analysis.p3_stage_b1_outer_cell_shard_v2 import (  # noqa: E402
    ANALYZER_BASIS_NAMES,
    ARTIFACT_TYPE,
    CELL_AUTHORIZATION,
    CELL_DATA_BOUNDARY,
...
def _parent_config_sha256(contract: P3StageB1ContractV2) -> str:
    return str(
        contract.raw["frozen_parent_bindings"]["d0_formal_stage_a_config"][
            "sha256"
        ]
    )
...
def _load_contract(path: Path) -> P3StageB1ContractV2:
    absolute = Path(os.path.abspath(os.fspath(path)))
    _repository_relative(absolute)
    contract = load_p3_stage_b1_contract_v2(absolute)
...
def _fixed_parent_paths(
    contract: P3StageB1ContractV2, dataset: str, condition: str
) -> tuple[Path, Path]:
...
def _fixed_aggregate_path(contract: P3StageB1ContractV2) -> Path:
...
def _preflight_parent_cell(
    *, contract: P3StageB1ContractV2, dataset: str, condition: str
    ...
    candidate = verify_label_free_shard(
        candidate_path,
        expected_config_sha256=_parent_config_sha256(contract),
...
def _coarse_group_layout(
    *, contract: P3StageB1ContractV2, worker: D0V3OuterSourceModel,
...
def _code_seal(contract: P3StageB1ContractV2) -> dict[str, Any]:
...
def _compute_cell(
    *, contract: P3StageB1ContractV2, dataset: str, condition: str,
    ...
    gradient_basis = contract.raw["gradient_basis"]
    if tuple(gradient_basis["direct_vjp_order"]) != STORAGE_BASIS:
        raise P3StageB1RunnerError("B1 v2 storage basis order differs from config")
    if int(gradient_basis["direct_vjp_count"]) != len(STORAGE_BASIS):
        raise P3StageB1RunnerError("B1 v2 direct VJP count differs from config")
    if (
        DIRECT_VECTOR_FIELDS != STORAGE_VECTOR_FIELDS
        or BACKWARD_BASIS != ANALYZER_BASIS_NAMES
        or len(BACKWARD_BASIS) != len(STORAGE_BASIS)
    ):
        raise P3StageB1RunnerError("analyzer VJP interface differs from storage")
    ...
    for image_index in range(image_count):
        ...
        try:
            ...
            audit = report.get("numeric_audit")
            if (
                not isinstance(audit, Mapping)
                or tuple(audit.get("direct_vjp_order", ())) != BACKWARD_BASIS
                or tuple(audit.get("direct_vector_fields", ()))
                != STORAGE_VECTOR_FIELDS
                or audit.get("direct_vjp_count") != len(STORAGE_BASIS)
                or audit.get("full_add_is_direct_vjp") is not True
            ):
                raise P3StageB1RunnerError(
                    f"analyzer numeric-audit interface differs at {image_index}"
                )
            ...
            basis_slice = np.stack(
                [getattr(vectors, field).numpy() for field in STORAGE_VECTOR_FIELDS],
                axis=0,
        ...
        gradient_integrity = build_gradient_integrity(
            np.ascontiguousarray(basis[image_index], dtype="<f4"),
            parent_entropy_gradients=np.ascontiguousarray(
                entropy_gradients[image_index], dtype="<f4"
            ),
            max_abs_tolerance=float(numeric["max_abs_tolerance"]),
            relative_l2_tolerance=float(numeric["relative_l2_tolerance"]),
        )
        if gradient_integrity["parent_direct_full_consistency_passed"] is not True:
            raise P3StageB1RunnerError(
                f"all-candidate parent entropy tolerance failed at {image_index}"
            )
        records.append(
...
def run_formal_cell(
    *, dataset: str, condition: str, config_path: Path, device: torch.device,
    ...
    canonical_destination = Path(
        os.path.abspath(
            os.fspath(_fixed_output_path(contract, dataset, condition))
        )
    )
    if output_path is not None:
        requested_destination = Path(os.path.abspath(os.fspath(output_path)))
        if requested_destination != canonical_destination:
            raise P3StageB1RunnerError(
                "formal Stage-B1 output_path must equal the frozen v2 cell path"
            )
    ...
    payloads = build_stage_b1_cell_payloads(
        ...
        execution={
            ...
            "direct_vjp_count": FORMAL_IMAGE_COUNT * len(STORAGE_BASIS),
...
def smoke_cell(
    *, dataset: str, condition: str, config_path: Path, device: torch.device
    ...
    contract = _load_contract(config_path)
    destination = _fixed_output_path(contract, dataset, condition)
    destination_existed_before = destination.exists() or destination.is_symlink()
    basis, records, _layout, _access, parent, runtime, code_seal = _compute_cell(
        contract=contract,
        dataset=dataset,
        condition=condition,
        device=device,
    ...
    if _code_seal(contract) != code_seal:
        raise P3StageB1RunnerError("B1 implementation changed during smoke")
    destination_exists_after = destination.exists() or destination.is_symlink()
    if destination_exists_after != destination_existed_before:
        raise P3StageB1RunnerError("B1 smoke changed the formal output path")
    first = records[0]
    return {
        "schema_version": SCHEMA_VERSION,
        ...
        "direct_vjp_count": len(STORAGE_BASIS),
        "source_logits_bit_exact": first["source_integrity"]["source_logits_bit_exact"],
        "parent_entropy_consistency_passed": first["gradient_integrity"][
...
def _collect_aggregate_preflight(
    *, contract: P3StageB1ContractV2, cell_code_seal: Mapping[str, Any]
) -> StageB1AggregatePreflight:

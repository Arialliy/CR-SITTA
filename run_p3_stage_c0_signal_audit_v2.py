...
DEFAULT_CONFIG: Final = REPOSITORY / "configs/p3_stage_c0_signal_audit_v2.yaml"
PROTOCOL_ID: Final = "cr-sitta-p3-stage-c0-signal-audit-v2"
PREDECESSOR_PROTOCOL_ID: Final = "cr-sitta-p3-stage-c0-signal-audit-v1"
PROBE_SEED_NAMESPACE: Final = PREDECESSOR_PROTOCOL_ID
...
def _mapping_with_exact_keys(
    ...
    """Validate an object roster without assigning order semantics to JSON keys."""

    mapping = _mapping(value, label)
    expected = frozenset(expected_keys)
    if len(mapping) != len(expected_keys) or frozenset(mapping) != expected:
        raise StageC0ProtocolError(f"{label} key roster differs")
    return mapping
...
def _validated_revision_lineage(
    repository: Path, raw: Mapping[str, Any]
) -> Mapping[str, Any]:
    lineage = _mapping_with_exact_keys(
        raw.get("revision_lineage"),
        (
            "predecessor_protocol_id",
            "predecessor_config",
            "predecessor_runner",
            "predecessor_pre_run_freeze",
            "predecessor_abort_receipt",
            "failed_engineering_artifact",
            "failure_reason_code",
            "repaired_scope",
            "formal_execution_started",
            "science_contract_changed",
            "science_gate_changed",
        ),
        "revision lineage",
    )
    if (
        lineage.get("predecessor_protocol_id") != PREDECESSOR_PROTOCOL_ID
        or lineage.get("failure_reason_code")
        != "canonical_json_mapping_order_mismatch"
        or lineage.get("repaired_scope")
        != "canonical_json_mapping_roster_validation_only"
        or lineage.get("formal_execution_started") is not False
        or lineage.get("science_contract_changed") is not False
        or lineage.get("science_gate_changed") is not False
    ):
        raise StageC0ProtocolError("revision lineage semantics differ")
    binding_fields = {
        "predecessor_config": ("path", "sha256"),
        "predecessor_runner": ("path", "sha256"),
        "predecessor_pre_run_freeze": ("path", "sha256"),
        "predecessor_abort_receipt": ("path", "sha256"),
        "failed_engineering_artifact": (
            "manifest_path",
            "manifest_sha256",
            "complete_path",
    ...
    for name, fields in binding_fields.items():
        binding = _mapping_with_exact_keys(lineage.get(name), fields, name)
        for field in fields:
            ...
            if field == "sha256":
                path_field = "path"
            digest = _require_sha256(binding.get(field), f"{name}.{field}")
            path = _repository_path(
                repository, binding.get(path_field), f"{name}.{path_field}"
            )
            if sha256_file(path) != digest:
                raise StageC0ProtocolError(f"revision binding changed: {name}")
    abort_path = _repository_path(
        repository,
        lineage["predecessor_abort_receipt"]["path"],
        "predecessor abort receipt",
    )
    abort = _load_json(abort_path)
    abort_failure = _mapping(abort.get("failure"), "predecessor abort failure")
    abort_formal = _mapping(
        abort.get("formal_execution"), "predecessor abort formal execution"
    )
    if (
        abort.get("protocol_id") != PREDECESSOR_PROTOCOL_ID
        or abort.get("status") != "aborted_before_formal_execution"
        or abort_failure.get("reason_code")
        != lineage["failure_reason_code"]
        or abort_formal.get("formal_execution_started") is not False
        or abort.get("scope", {}).get("gate_evaluated") is not False
        or abort.get("scope", {}).get("scientific_result") is not False
    ):
        raise StageC0ProtocolError("predecessor abort receipt semantics differ")
    return lineage
...
def _validate_contract_semantics(repository: Path, raw: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "python run_p3_stage_c0_signal_audit_v2.py freeze"
        or freeze.get("verify_command")
        != "python run_p3_stage_c0_signal_audit_v2.py verify-config"
        or freeze.get("print_hash_command")
        != "python run_p3_stage_c0_signal_audit_v2.py print-config-sha256"
    ):
        raise StageC0ProtocolError("config freeze contract differs")
    _validated_revision_lineage(repository, raw)
    ...
    if (
        ...
        or bank.get("probe_seed_descriptor_protocol_field_name") != "protocol_id"
        or bank.get("probe_seed_namespace_role")
        != "preserve_v1_scientific_inputs_bit_exact"
    ...
    expected_output = {
        "root": "results/cr_sitta/p3_stage_c0_signal_audit_v2",
...
def _expected_pre_run_freeze_receipt(
    ...
    return {
        ...
        "revision_lineage": dict(
            _validated_revision_lineage(contract.repository, contract.raw)
        ),
        "probe_seed_contract": {
            "artifact_protocol_id": PROTOCOL_ID,
            "descriptor_protocol_field_name": "protocol_id",
            "descriptor_protocol_field_value": PROBE_SEED_NAMESPACE,
...
def derive_probe_seed(
    *,
    ...
    if probe_seed_namespace != PROBE_SEED_NAMESPACE:
        raise StageC0ProtocolError("probe seed namespace differs from predecessor")
    ...
    descriptor = {
        # Preserve the predecessor descriptor bytes exactly.  The v2 artifact
        # protocol ID is intentionally not part of the scientific probe seed.
        "protocol_id": probe_seed_namespace,
            ...
            for image_index in range(image_count):
                ...
                for probe_index, probe in enumerate(contract.probe_records):
                    probe_seed = derive_probe_seed(
                        probe_seed_namespace=str(
                            contract.raw["deterioration_bank"][
        ...
        manifest = {
            **_runtime_manifest_base(
            ...
            "method_boundary": {
                ...
                "probe_seed_namespace": PROBE_SEED_NAMESPACE,
                "probe_seed_descriptor_protocol_field_name": "protocol_id",
                "probe_seed_namespace_preserves_v1_scientific_inputs": True,
            },
            "identity_adapter": {
...
def _validate_layouts(value: Any) -> dict[str, Mapping[str, Any]]:
    layouts = _mapping_with_exact_keys(
        value, PARAMETER_SPACES, "parameter layouts"
    )
...
def _validate_manifest_freeze_binding(
    ...
    if (
        ...
        or manifest.get("implementation_revision") != 2
        or manifest.get("predecessor_protocol_id") != PREDECESSOR_PROTOCOL_ID
        or manifest.get("probe_seed_namespace") != PROBE_SEED_NAMESPACE
    ):
...
def verify_candidate_artifact(
    ...
    *,
    ...
    if (
        ...
        or boundary.get("probe_seed_namespace") != PROBE_SEED_NAMESPACE
        or boundary.get("probe_seed_descriptor_protocol_field_name")
        != "protocol_id"
        or boundary.get("probe_seed_namespace_preserves_v1_scientific_inputs")
        is not True
    ):
    ...
    for condition_index, (corruption, severity) in enumerate(condition_grid):
        for image_index in range(image_count):
            for probe_index, probe_id in enumerate(PROBE_IDS):
                ...
                if (
                    record.get("probe_seed_namespace") != PROBE_SEED_NAMESPACE
                    or record.get("probe_seed")
                    != derive_probe_seed(
                        probe_seed_namespace=PROBE_SEED_NAMESPACE,
                        global_seed=int(contract.raw["scope"]["seed"]),
                        image_id=str(record.get("image_id")),
                        probe_id=probe_id,
                        input_tensor_sha256=str(record.get("input_tensor_sha256")),
                    )
                ):
                    raise StageC0ProtocolError("candidate probe seed differs")
                evidence = _mapping_with_exact_keys(
                    record.get("gradient_evidence"),
                    PARAMETER_SPACES,
                    "candidate gradient evidence",
                )
                for space in PARAMETER_SPACES:
...
def verify_outer_artifact(
    ...
    *,
    ...
    for index, record in enumerate(outer):
        ...
        spaces = _mapping_with_exact_keys(
            record.get("parameter_space_evidence"),
            PARAMETER_SPACES,
...
def build_aggregate_evidence(
    ...
    *,
    ...
    _mapping_with_exact_keys(
        outer_records_by_dataset, DATASETS, "aggregate outer-record datasets"
    )
    _mapping_with_exact_keys(
        identity_by_dataset, DATASETS, "aggregate identity datasets"
    )
...
def verify_aggregate_artifact(
    ...
    outer_bindings = _mapping_with_exact_keys(
        manifest.get("outer_artifacts"),
        DATASETS,
        "aggregate outer artifacts",
    )
    outer_records: dict[str, Sequence[Mapping[str, Any]]] = {}
    identities: dict[str, Mapping[str, Any]] = {}
    for dataset in DATASETS:

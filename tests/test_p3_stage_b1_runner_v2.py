...
from analysis.foreground_background_gradient_decomposition_v2 import GROUP_IDS
from analysis.p3_stage_b1_contract_v2 import load_p3_stage_b1_contract_v2
from analysis.p3_stage_b1_outer_cell_shard_v2 import BASIS_ORDER
...
def _successful_analysis(*_args: Any, **_kwargs: Any) -> Any:
    ...
    return SimpleNamespace(
        vectors=SimpleNamespace(
            ...
            full_direct=sub + supra + background,
            foreground_total_direct=sub + supra,
            foreground_subthreshold_direct=sub,
            foreground_suprathreshold_raw_direct=supra,
            background_raw_direct=background,
        ),
        report={
            "target_statistics": {
            ...
            "numeric_audit": {
                ...
                "direct_vjp_order": list(runner.BACKWARD_BASIS),
                "direct_vector_fields": list(runner.STORAGE_VECTOR_FIELDS),
                "direct_vjp_count": 5,
...
def test_existing_formal_destination_blocks_compute_before_side_effect(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    destination = tmp_path / "existing" / "cell"
    destination.mkdir(parents=True)
    contract = SimpleNamespace()
    monkeypatch.setattr(runner, "_load_contract", lambda _path: contract)
    monkeypatch.setattr(runner, "_fixed_output_path", lambda *_args: destination)
    monkeypatch.setattr(runner, "_repository_relative", lambda _path: "cell")
    monkeypatch.setattr(runner, "_compute_cell", _failing)
...
def test_formal_output_override_cannot_escape_frozen_v2_cell_path(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    contract = SimpleNamespace()
    canonical = tmp_path / "frozen" / "cell"
    requested = tmp_path / "alternate" / "cell"
    monkeypatch.setattr(runner, "_load_contract", lambda _path: contract)
    monkeypatch.setattr(runner, "_fixed_output_path", lambda *_args: canonical)
    monkeypatch.setattr(runner, "_compute_cell", _failing)

    with pytest.raises(
        runner.P3StageB1RunnerError,
        match="must equal the frozen v2 cell path",
    ):
        runner.run_formal_cell(
            dataset=DATASET,
            condition=CONDITION,
...
def test_smoke_rechecks_code_seal_and_never_calls_publication_writers(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    destination = tmp_path / "formal" / "cell"
    seal = {"files": [], "bundle_sha256": "a" * 64}
    contract = SimpleNamespace()
    parent = SimpleNamespace(ordered_image_ids=("pilot_000",))
    record = {
        "source_integrity": {
        ...
        "gradient_integrity": {
            "parent_direct_full_consistency_passed": True,
        },
    }
    monkeypatch.setattr(runner, "_load_contract", lambda _path: contract)
    monkeypatch.setattr(runner, "_fixed_output_path", lambda *_args: destination)
    monkeypatch.setattr(
        ...
        "_compute_cell",
        lambda **_kwargs: (
            np.zeros((1, 5, runner.SCALAR_COUNT), dtype="<f4"),
            [record],
            {},
            b"{}",
            parent,
    ...
    monkeypatch.setattr(runner, "_code_seal", lambda _contract: seal)
    monkeypatch.setattr(runner, "_write_bytes", _failing)
    monkeypatch.setattr(runner, "_write_npy", _failing)
    monkeypatch.setattr(runner.tempfile, "mkdtemp", _failing)
    monkeypatch.setattr(runner, "publish_flat_directory_noreplace", _failing)

    result = runner.smoke_cell(
        dataset=DATASET,
        condition=CONDITION,
        config_path=tmp_path / "config.yaml",
        device=torch.device("cpu"),
    ...
    assert result["direct_vjp_count"] == 5
    assert result["filesystem_created"] is False
...
def test_receipt_validator_is_fail_closed_before_code_cuda_and_target(
    ...
    contract = SimpleNamespace(
        ...
        raw={
            "frozen_parent_bindings": {
                "cache_protocol": {"sha256": "8" * 64},
                "d0_formal_stage_a_config": {"sha256": "0" * 64},
            },
...
def test_analyzer_direct_vjp_interface_mismatch_fails_closed_and_resets(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    entropy = np.full((1, 10, runner.SCALAR_COUNT), 6.0, dtype="<f4")

    def mismatched_analysis(*args: Any, **kwargs: Any) -> Any:
        result = _successful_analysis(*args, **kwargs)
        result.report["numeric_audit"]["direct_vjp_order"] = list(
            reversed(runner.BACKWARD_BASIS)
        )
        return result

    contract, state, _events = _install_compute_harness(
        monkeypatch,
        analyzer=mismatched_analysis,
        parent_entropy=entropy,
    )
    with pytest.raises(runner.P3StageB1RunnerError, match="numeric-audit interface"):
        runner._compute_cell(
            contract=contract,
            dataset=DATASET,
            condition=CONDITION,
...
def test_basis_storage_and_group_order_are_exact() -> None:
    ...
    assert tuple(contract.raw["gradient_basis"]["direct_vjp_order"]) == BASIS_ORDER
...
def test_basis_array_is_stored_in_frozen_order(
    ...
    assert basis.shape == (1, 5, runner.SCALAR_COUNT)
    assert np.all(basis[0, 0] == 6.0)
    assert np.all(basis[0, 1] == 3.0)
    assert np.all(basis[0, 2] == 1.0)
    assert np.all(basis[0, 3] == 2.0)
    assert np.all(basis[0, 4] == 3.0)
    assert records[0]["gradient_integrity"]["cumulative_basis_order"] == list(
        BASIS_ORDER
    )
    assert state.reset_calls == 1
...
def test_code_seal_is_rechecked_before_any_output_directory_is_created(
    ...
    monkeypatch.setattr(
        ...
        "_compute_cell",
        lambda **_kwargs: (
            np.zeros((64, 5, runner.SCALAR_COUNT), dtype="<f4"),
            [],
            {},
            b"{}",
            parent,
...
def test_formal_publication_uses_final_builder_and_verifier_interfaces(
    ...
    basis = np.zeros((64, 5, runner.SCALAR_COUNT), dtype="<f4")
